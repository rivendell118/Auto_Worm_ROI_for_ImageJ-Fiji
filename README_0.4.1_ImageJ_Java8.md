...
>
>当前版本（0.4.1 CUDA for ImageJ（Java 8））是为使用NVIDIA系列显卡，且  ImageJ 内置 Java 版本为 Java 8 的用户开发的测试版。具体详见 [测试版说明](#测试版说明)
>
...
0.4.1 CUDA for ImageJ（Java 8）在开发机上跑通了大部分功能。但是，部分异常情况因缺乏数据仅进行了代码审查，未能进行真实数据测试。此外，可能还存在未能够考虑到的异常情况，需要用户进行反馈。
...
<Fiji 根目录>/
  ...
  plugins/
    ...
    AutoWormImageJ/
      ...
      licenses/              第三方组件许可全文
  README_0.4.1_ImageJ_Java8.md     本文件
  CHANGELOG_0.4.1.md
  VALIDATION_0.4.1.md
  LICENSE
```
...
结果默认写在 **当前文件夹下的 `_auto_roi` 子目录** 里，可以在设置界面里改。
...
每次处理都在输出文件夹写入 `batch_summary.csv`，每处理完一张刷新一次，所以中途取消或出错也能拿到已完成的部分。
...
| 文件 | 写出方 | 内容 |
| --- | --- | --- |
| `batch_summary.csv` | 圈虫 | 每张图一行的总表：识别数量、`qc_status`、耗时、失败原因（`error` 列） |
| `<图名>_RoiSet.zip` | 圈虫 | 该图的 ROI 集合，ImageJ 读它做测量 |
| `<图名>_QC.png` | 圈虫 | 圈画结果预览图，叠出分割结果并写上 QC 状态，建议抽看 |
| `<图名>_measurements.csv` | ImageJ | 该图的测量表 |
| `ImageJ_measurements_all.csv` | ImageJ | 本批所有图的合并测量表 |
| `<图名>_head_annotations.csv` | 圈虫 | 头向标注的匹配结果 |
| `<图名>_manual_split_qc.csv` | 圈虫 | 人工分界精修明细，仅在该图的分界真正改变了像素归属时写出 |
| `<图名>_shape_qc.csv` | 圈虫 | 平滑修复前后的轮廓质量 |
| `<图名>_split_qc.csv` | 圈虫 | 低清晰度模式下的粘连拆分明细 |
| `<图名>_segment_qc.csv` | 圈虫 | 部分圈画的选段明细 |

后四张 QC 明细表**只在与它对应的功能开启时才写出**；未开启时会顺带删掉上一次运行留下的同名文件，免得看到过期数据。
...
少的那张处理失败了。到输出文件夹的 `batch_summary.csv` 里找 `qc_status` 为 `FAILED` 的行，`error` 列就是原因。
...
**找不到结果。**
默认在**输入图像文件夹下的 `_auto_roi` 子目录**里，也可以在界面里改成别的路径。
...
```bat
build_0.4.1.bat "完整路径\python.exe"
```
